...
{
  $req_profil_data = $mysql->prepare("SELECT * FROM media_users WHERE id=:id");
  $req_profil_data->bindValue(':id', intval($_GET['id']), PDO::PARAM_STR);
  $req_profil_data->execute();
  $profil_data = $req_profil_data->fetch();
  ...
  if (verify_token( $_POST['myprofil_hash'], $_POST['myprofil_CSRF']) == 1)
  {
    ...
    if(isset($_POST['use_otp']) && $_POST['use_otp'] == 1 && $_POST['use_otp'] != $profil_data['is_OTP_activated'])
    {
      ...
      $req_changelog_user_update->bindValue(':id', intval($_GET['id']), PDO::PARAM_STR);
    ...
    if((isset($_POST['change_password']) && $_POST['change_password'] == 1) && (isset($_POST['actual_password']) && !empty($_POST['actual_password']) && isset($_POST['new_password']) && !empty($_POST['new_password']) && isset($_POST['confirm_new_password']) && !empty($_POST['confirm_new_password'])))
    {
      if($_POST['confirm_new_password'] == $_POST['new_password'])
      {
        ...
        $pass = Generate_Popup_result(0, 'Changement de mot de passe du compte effectué avec succès');
      }
      else
      {
    ...
    if (isset($_POST['myprofil_hash']) && !empty($_POST['myprofil_hash']) && isset($_POST['myprofil_CSRF']) && !empty($_POST['myprofil_CSRF']) && isset($_POST['pseudo']) && !empty($_POST['pseudo']))
    {
      ...
      $req_changelog_update_user->bindValue(':id', intval($_GET['id']), PDO::PARAM_STR);
      ...
      $msg = Generate_Popup_result(0, 'Compte a été mis à jour avec succès !');
    }
    else if (isset($_POST['myprofil_hash']) OR isset($_POST['myprofil_CSRF']) OR isset($_POST['pseudo']))
    {
      ...
      $msg = Generate_Popup_result(2, 'Une erreur indéterminée est survenue lors de la soumission de la mise a jour du compte, il n\'a pas été enregistré :(');
    }
  }
  else
  {
...
$req_profil_view_data->bindValue(':id', intval($_GET['id']), PDO::PARAM_STR);
...
?>
      ...
      <div class="panel-body">
                ...
                <form method="POST" action="/admin/edit_account.php?id=<?php echo intval($_GET['id']); ?>" accept-charset="UTF-8">
                <div class="row">
                  <div class="col-md-12">
                    ...
                    <div class="form-group">
                      <label class="sr-only" for="exampleInputAmount">Changer le mot de passe du compte : </label>
                      <div class="input-group">
                        <div class="input-group-addon">Changer le mot de passe du compte : </div>
                        <select class="form-control form-control-select" id="change_password" name="change_password" data-toggle="tooltip" data-html="true" data-original-title="<b>/!\ ATTENTION :</b> Changer de mot de passe entraine une déconnexion automatique de la personne dès la mise a jour de celui !">
