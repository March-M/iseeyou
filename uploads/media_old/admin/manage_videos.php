...
?>
      <div class="login-panel-heading">
        <h3 class="login-panel-title center">Gérer les vidéos</h3>
      </div>
      ...
      <table class="table table-striped" style="table-layout: fixed; word-wrap: break-word;">
        ...
        <tbody>
          ...
          $req_data_video = $mysql->prepare("SELECT * FROM media_list ORDER BY id DESC");
          $req_data_video->execute();

          //On traite le tout pour l'affichage
          while ($data_video = $req_data_video->fetch())
          {
            ?>

            <tr>
              <td><?php echo $data_video['id']; ?></td>
              <td><?php echo $data_video['press_name']; ?></td>
              <td><?php echo $data_video['press_date'] ?></td>
              <td><a href="edit_video.php?id=<?php echo $data_video['id']; ?>" class="btn btn-success btn-sm" role="button">Modifier</a> <a href="remove_video.php?id=<?php echo $data_video['id']; ?>" class="btn btn-danger btn-sm" role="button">Supprimer</a></td>
              <?php
          }
          ?>
          </tbody>
...
<?php
include($_SERVER['DOCUMENT_ROOT'].'/media_42/inc/footer.php');
?>
