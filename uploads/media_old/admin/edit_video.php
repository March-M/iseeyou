...
if (isset($_POST['press_name']) && isset($_POST['press_date']) && isset($_POST['thumbnail']) && isset($_POST['video_mp4']) && isset($_POST['video_webm']) && isset($_POST['video_ogg']) && isset($_POST['video_hash']) && isset($_POST['video_CSRF']) && !empty($_POST['press_name']) && !empty($_POST['press_date']) && !empty($_POST['thumbnail']) && !empty($_POST['video_mp4']) && !empty($_POST['video_webm']) && !empty($_POST['video_ogg']) && !empty($_POST['video_hash']) && !empty($_POST['video_CSRF']))
{
  ...
  if (verify_token($_POST['video_hash'], $_POST['video_CSRF']) == 1)
  {
    ...
    if ($_SESSION['token']['article'] != $_POST['video_hash'])
    {
      ...
      $req_article_add_comment = $mysql->prepare("UPDATE `media_list` SET `press_name`=:pressname,`press_date`=:pressdate,`thumbnails`=:thumbnail,`video_mp4`=:mp4,`video_webm`=:webm,`video_ogg`=:ogg WHERE `id` = :id");
      $req_article_add_comment->bindValue(':id', intval($_GET['id']), PDO::PARAM_STR);
...
if(isset($msg) && $msg != "")
{
  echo $msg."<br />";
      ...
      $req_video_data = $mysql->prepare("SELECT * FROM media_list WHERE id=:id ");
      $req_video_data->bindValue(':id', intval($_GET['id']), PDO::PARAM_STR);
      $req_video_data->execute();
      $video_data = $req_video_data->fetch();
?>
      <div class="login-panel-heading">
        <h3 class="login-panel-title center">Editer une vidéo</h3>
      </div>
      <div class="panel-body">
        <form method="POST" action="/admin/edit_video.php?id=<?php echo intval($_GET['id']); ?>" accept-charset="UTF-8">
          <div class="input-group-addon">Nom de l'article</div>
          <input type="text" id="press_name" class="form-control login-panel-input" name="press_name" placeholder="Nom de l'article de presse" required="" autofocus="" value="<?php echo $video_data['press_name']; ?>">
          <div class="input-group-addon">Date</div>
            <input type="text" id="press_date" class="form-control login-panel-input" name="press_date" placeholder="Date de publication" required="" autofocus="" value="<?php echo $video_data['press_date']; ?>">
          <div class="input-group-addon">Thumbnail</div>
            <input type="hidden" id="thumbnail" name="thumbnail" required="" value="<?php echo $video_data['thumbnails']; ?>">
            <?php
              simple_upload_form('thumbnail');
            ?>
          <div class="input-group-addon">MP4</div>
            <input type="hidden" id="video_mp4" name="video_mp4" required="" value="<?php echo $video_data['video_mp4']; ?>">
            <?php
              simple_upload_form('video_mp4');
            ?>
          <div class="input-group-addon">WEBM</div>
            <input type="hidden" id="video_webm" name="video_webm" required="" value="<?php echo $video_data['video_webm']; ?>">
            <?php
              simple_upload_form('video_webm');
            ?>
          <div class="input-group-addon">OGG</div>
            <input type="hidden" id="video_ogg" name="video_ogg" required="" value="<?php echo $video_data['video_ogg']; ?>"/>
            <?php
  ...
  <button type="submit" name="submit" class="btn btn-info btn-block">Enregistrer les modifications</button>
        </form>
              </div>
    </div>
  </div>
